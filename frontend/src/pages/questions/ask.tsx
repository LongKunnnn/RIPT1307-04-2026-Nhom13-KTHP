  ...
  const onFinish = async (values: CreatePostInput) => {
    ...
    try {
      ...
      if (post.moderationStatus === 'published') {
        notify.notifyEmail('Bài đăng mới', `Bài "${post.title}" đã được đăng (email giả lập).`);
      }
      message.success(moderationUserMessage(post.moderationStatus, post.moderationFlags));
      form.resetFields();
      ...
      <div className={styles.page}>
        <div className={styles.inner}>
          <Card>
            ...
            <Button type="primary" style={{ marginTop: 12 }} onClick={() => history.push(ROUTES.login)}>
              Đăng nhập
            </Button>
          </Card>
        </div>
      ...
      <div className={styles.inner}>
        <div className={styles.head}>
          ...
          <Text className={styles.sub}>
            Mô tả rõ vấn đề, đã thử gì và kỳ vọng điều gì — giúp mọi người trả lời nhanh và đúng trọng tâm.
          </Text>
        </div>
        ...
        <Card className={styles.card} bordered={false}>
          ...
          >
            ...
            >
              <Input showCount maxLength={200} placeholder="Tóm tắt vấn đề một cách cụ thể" />
            </Form.Item>
