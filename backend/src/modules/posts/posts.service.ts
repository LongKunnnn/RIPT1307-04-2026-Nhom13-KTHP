...
export class PostsService {
  ...
  async mapPost(post: PostWithRelations, viewerId?: number) {
    const matchedWords = Array.isArray(post.matched_words)
      ? (post.matched_words as unknown as string[])
      : undefined;
    ...
    return {
      ...
      acceptedCommentId: post.accepted_comment_id
        ? String(post.accepted_comment_id)
        : undefined,
  ...
  async list(query: ListPostsQueryDto, viewer?: AuthUserPayload | null) {
    ...
    } else if (!query.includeNonPublic) {
      where.moderation_status = ModerationStatus.published;
    }
    ...
    if (query.tag?.trim()) {
      where.post_tags = {
        some: { tag: { name: { equals: query.tag.trim() } } },
      };
    }

    if (query.difficulty) {
      where.difficulty = query.difficulty as PostDifficulty;
    }
      ...
      const withActivity = await Promise.all(
        all.map(async (p) => {
          const lastComment = await this.prisma.comment.findFirst({
            where: { post_id: p.id, deleted_at: null },
            orderBy: { created_at: 'desc' },
          });
          const activity = lastComment
            ? Math.max(p.created_at.getTime(), lastComment.created_at.getTime())
    ...
    const mapped = await Promise.all(
      items.map((p) => this.mapPost(p, viewer?.id)),
    );
  ...
  async getById(id: number, viewer?: AuthUserPayload | null, admin = false) {
    ...
    if (
      !admin &&
      post.moderation_status !== ModerationStatus.published &&
      !isAuthor
    ) {
    ...
    if (bounty > 0) {
      ...
      if (!author || author.reward_points < bounty) {
        throw new BadRequestException(
          `Không đủ điểm thưởng để gắn thưởng. Cần ${bounty}, bạn có ${author?.reward_points ?? 0}.`,
        );
      }
    }
    ...
    const post = await this.prisma.$transaction(async (tx) => {
      if (bounty > 0) {
        await tx.user.update({
          where: { id: user.id },
          data: { reward_points: { decrement: bounty } },
        });
      }
      return tx.post.create({
        data: {
          ...
          matched_words: scan.matchedWords.length
            ? scan.matchedWords
            : undefined,
          post_tags: {
            create: await this.resolveTags(tagNames),
          },
        },
        include: { author: true, post_tags: { include: { tag: true } } },
      });
  ...
  async update(id: number, dto: UpdatePostDto) {
    ...
    if (dto.tags) {
      const tagNames = Array.from(
        new Set(dto.tags.filter((t) => t.trim().length > 0)),
      );
  ...
  async ratePost(postId: number, userId: number, stars: number) {
    ...
    if (post.author_id === userId) {
      throw new BadRequestException('Không thể tự đánh giá bài của mình');
    }
    ...
    const agg = await this.prisma.postRating.aggregate({
  ...
  async getMyRating(postId: number, userId: number) {
    const row = await this.prisma.postRating.findUnique({
      where: { post_id_user_id: { post_id: postId, user_id: userId } },
    });
  ...
  async acceptAnswer(postId: number, commentId: number, authorId: number) {
    const post = await this.prisma.post.findFirst({
      where: { id: postId, deleted_at: null, author_id: authorId },
    });
    if (!post)
      throw new NotFoundException(
        'Không tìm thấy bài viết hoặc bạn không phải tác giả',
      );
    ...
    if (!comment)
      throw new NotFoundException('Không tìm thấy câu trả lời hợp lệ');
    if (post.accepted_comment_id) {
      throw new BadRequestException('Đã chấp nhận một câu trả lời rồi');
    }
    ...
    await this.prisma.$transaction(async (tx) => {
      if (bounty > 0) {
        await tx.user.update({
          ...
          data: { reward_points: { increment: bounty } },
        });
        await tx.post.update({
          where: { id: postId },
          data: { bounty: 0, accepted_comment_id: commentId },
        });
      } else {
        await tx.post.update({
          where: { id: postId },
          data: { accepted_comment_id: commentId },
        });
      }
    ...
    return {
      acceptedCommentId: String(commentId),
      bountyAwarded: bounty,
      answererId: String(comment.author_id),
    };
  }
  ...
  async getTagsWithCount() {
    ...
    return tags
      .map((t) => ({ name: t.name, count: t.post_tags.length }))
      .filter((t) => t.count > 0)
